...
for id in IDs:
    ...
    # run example code
    fiducials,s=ppg_example(data_path=data_path, start_sig=start_sig, end_sig=end_sig, process_type="fiducials", savingfolder=savingfolder, savefig=True, show_fig=False)
    ...
    for key in fp_keys:
        ...
        try:
            sqi =  round(np.mean(SQI.get_ppgSQI(s.get_s()[tmp_sig].values, s.fs, fiducials[key]- s.start_sig)) * 100, 2)
        except:
            sqi = np.NAN
